...
const BOOKS_DB_KEY = 'booksDb';
var gBooks = storageService.loadFromStorage(BOOKS_DB_KEY) || _createBooks();
...
function getBookReviews(bookId) {
  var book = gBooks.find(book => book.bookId === bookId);
  return book.reviews;
}
...
function saveReview(bookId, review) {
  ...
  storageService.storeToStorage(BOOKS_DB_KEY, gBooks);
}

function getEmptyReview() {
  return Promise.resolve({ id: utilService.makeId(), name: '', text: '', rating: 0 });
}

function getBooks() {
  return new Promise((resolve) => {
    gBooks = storageService.loadFromStorage(BOOKS_DB_KEY);
    resolve(gBooks);
  })
}
...
function addBook(result) {
  var newBook = convertFormat(result);
  gBooks.push(newBook);
  storageService.storeToStorage(BOOKS_DB_KEY, gBooks);
}
...
function getNextBookId(bookId,diff){
  ...
  if(index === -1) return gBooks[0];
  if(index+diff < 0 || index+diff > gBooks.length-1) return gBooks[index].id;
  return gBooks[index+diff].id;
}

function convertFormat(result) {
  console.log(result);
  var authors = [];
  var categories = [];

  if(result.volumeInfo.authors){
    authors = result.volumeInfo.authors.slice();
  }
  if(result.volumeInfo.categories){
    categories = result.volumeInfo.categories.slice();
  }

  //TODO: add description from google result.volumeInfo.description
  var newBook = {
    "id": result.id,
    "reviews": [],
    ...
    "authors": authors,
    "publishedDate": result.volumeInfo.publishedDate.split("-")[0],
  ...
  function _createBooks() {
    ...
    storageService.storeToStorage(BOOKS_DB_KEY, books);
    return books;
  }